...
public class EmailsList extends Activity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //set SearchView
        searchView = (SearchView) findViewById(R.id.searchEmailsInput);
        searchView.setOnQueryTextListener(this);
        searchView.setOnCloseListener(this);

        //loading the fonts and apply them
        Thin = Typeface.createFromAsset(getAssets(), "fonts/MyriadSetPro-Thin.ttf");
        TextView contactsText = (TextView) findViewById(R.id.emailsText);
        contactsText.setTypeface(Thin);

        //get contacts
        contacts = new ArrayList<Contact>();
        contacts = dbHandler.getContactsWithEmails();

        //covert array to list items
        adapter = new CustomAdapter(this, contacts, mTouchListener, false);
        ListView = (ListView) findViewById(R.id.ContactsListView);
        ListView.setAdapter(adapter);

        //set a listener on click search
        /*
        centerButton = (ImageView) findViewById(R.id.contactsButton);
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        searchResults = dbHandler.searchByNameWithEmail(textToSearch);

        //covert array to list items
        adapter = new CustomAdapter(this, searchResults, mTouchListener, false);
        ListView = (ListView) findViewById(R.id.ContactsListView);
        ListView.setAdapter(adapter);
    ...
    private View.OnTouchListener mTouchListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(final View v, MotionEvent event) {
            ...
            if (mSwipeSlop < 0) {
                mSwipeSlop = ViewConfiguration.get(EmailsList.this).
                        getScaledTouchSlop();
            }
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                {
                    ...
                    if (mSwiping) {
                        ...
                        if((double)(System.nanoTime() - start_time)/1000000000 <= 0.2) {
                            int p = ListView.getPositionForView(v);
                            String Email = adapter.getItem(p).getEmail();
                            String nameToUpdate = adapter.getItem(p).getName();
                            dbHandler.incrementNumEmails(nameToUpdate);
                            sendEmail(Email);
                        }
                        v.setBackgroundColor(Color.WHITE);
                        start_time = 0 ;
    ...
    protected void sendEmail(String To) {
        ...
        try {
            ...
            Toast.makeText(EmailsList.this, "There are no email clients installed.", Toast.LENGTH_SHORT).show();
        }
